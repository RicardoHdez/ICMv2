    ...
    <table border="0" cellpadding="2" cellspacing="2">
        ...
        <form accept-charset="utf-8" method="POST" action="estadocuenta_buscar.php">
            ...
            <?php
                ...
                @mysql_query ($SQL);
                //Variables para trabajar con filtro de busqueda
                $Plantel = $_POST['Seleccion_Plantel'];
                $Turno = $_POST['Turno'];
                $Buscador = $_POST['B_Nombre'];
                //Consula de tabla alumnos
                $consulta_alumnos = mysql_query("select * from alumnos");
                if(!$Buscador){ //Condicion: si la variable Buscar se encuentra vacia
                    if($Plantel == "PLANTEL"){ //Condicion: si se seleccionan todos
                        if($Turno == "TURNO"){ $consulta_alumnos = mysql_query("select * from alumnos"); }
                        else{$consulta_alumnos = mysql_query("select * from alumnos where turno = '".$Turno."'");}
                    }else{
                        if($Turno == "TURNO"){ $consulta_alumnos = mysql_query("select * from alumnos where escuela='".$Plantel."'"); }
                        else{$consulta_alumnos = mysql_query("select * from alumnos where escuela = '".$Plantel."' and turno = '".$Turno."'");}
                    }
                }else{ //Caso contrario de que la variable Buscar contenga algo
                    if($Plantel == "PLANTEL"){
                        if($Turno == "TURNO"){ $consulta_alumnos = mysql_query("select * from alumnos where nom_completo like '%".$Buscador."%'"); }
                        else{$consulta_alumnos = mysql_query("select * from alumnos where nom_completo like '%".$Buscador."%' and turno = '".$Turno."'");}
                    }else{
                        if($Turno == "TURNO"){ $consulta_alumnos = mysql_query("select * from alumnos where nom_completo like '%".$Buscador."%' and escuela like '".$Plantel."'"); }
                        else{$consulta_alumnos = mysql_query("select * from alumnos where nom_completo like '%".$Buscador."%' and turno = '".$Turno."' and escuela like '".$Plantel."'");}
                    }
                }
